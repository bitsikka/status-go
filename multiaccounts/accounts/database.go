...
type Settings struct {
	// required
	Address                   types.Address    `json:"address"`
	BrightidLinkStatus        *json.RawMessage `json:"brightid-link-status,omitempty"`
	BrightidLinkErrorStatus   *json.RawMessage `json:"brightid-link-error-status,omitempty"`
...
func (db *Database) SaveSetting(setting string, value interface{}) error {
	...
	switch setting {
	case "brightid-link-status":
		value = &sqlite.JSONBlob{value}
		update, err = db.db.Prepare("UPDATE settings SET brightid_link_status = ? WHERE synthetic_id = 'id'")
	case "brightid-link-error-status":
		value = &sqlite.JSONBlob{value}
		update, err = db.db.Prepare("UPDATE settings SET brightid_link_error_status = ? WHERE synthetic_id = 'id'")
	case "chaos-mode?":
		_, ok := value.(bool)
		if !ok {
